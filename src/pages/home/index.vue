...
function handleItemClick(e) {
  ...
  listStore.updateScrollTop(listEl.value.scrollTop)
  router.push({
    name: 'detail',
    params: { id },
...
onMounted(() => {
  if (listStore.list.length && listStore.scrollTop) {
    listEl.value.scroll(0, listStore.scrollTop)
  }
})
</script>
...
<template>
  <div class="container">
    ...
    <div class="list" @click="handleItemClick" ref="listEl">
      <van-pull-refresh v-model="state.refreshing" @refresh="refresh">
        <van-list
          ...
          @load="load"
        >
          <div class="card" v-for="item in listStore.list" :key="item.id" :data-ani-id="item.id">
            <div class="title">
              {{ item.title }}
            </div>
            <div class="info">
